...
import { Data, RecipeData, Recipes } from "./data/data.model.js";
...
const elRootChainedRatios: HTMLElement = document.getElementById("SelectedRatios") as HTMLElement;
...
function renderRatioData(): void {
  ...
  if (!data_fuel_generator_rates) throw new Error("data_fuel_generator_rates is null");
  if (!data_coal_generator_rates) throw new Error("data_coal_generator_rates is null");
  if (!data_biomass_generator_rates) throw new Error("data_biomass_generator_rates is null");
  ...
  if (!data_alien_power_augmentor_rates) throw new Error("data_lien_power_augmentor_rates is null");
...
function renderCalculatedRatios(data: { recipes: Recipes; name: string }[]): void {
  data.forEach((data) => {
...
function parseData(rawRecipesData: Recipes, name: string): ProductionItemSet[] {
  // console.log(`____________ Parse ${name} _______________`);
  // console.log(rawRecipesData);
  let data: ProductionItemSet[] = [];

  // Loop over recipes
  Object.entries(rawRecipesData).forEach(([recipeName, recipeData]: [string, RecipeData], i: number): void => {
    ...
    const firstProductRate: number | undefined = Object.values(recipeData.out)[0];
    let num: number = firstProductRate !== undefined ? Number(firstProductRate) : 1;
    ...
    const ratesRequired: number[] = Object.entries(recipeData.in).map(
    ...
    // Ins
    Object.entries(recipeData.in).forEach(([item, itemRequiredRate]: [string, number]): void => {
      // console.log(`----- ${item} -----`);
      let itemRecipeName: string = item;
      let itemRecipe: RecipeData = rawData[itemRecipeName];
    ...
    let ratioMap: number[][] = ratesRequired.map((rate: number, i: number): number[] =>
      getRatioSimple(rate, ratesProvided[i])
    );
